...
public class Puzzle extends Activity implements View.OnDragListener, View.OnTouchListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Log.i("size_tag", puzzleSize + " " + allPieces.size());
    ...
    private void fromTypedArrayToList() {
        ...
        TypedArray typedArray = getResources().obtainTypedArray(R.array.images4x4);
    ...
    public boolean onDrag(View view, DragEvent dragEvent) {
        ...
        switch (action) {
            case DragEvent.ACTION_DROP: {
                ...
                animation(dragImage, onDragImage);
    ...
    private void storingInformation(double timer) {
    ...
    private void animation(ImageView dragImage, ImageView onDragImage) {
        Point onDragPoint = (Point) onDragImage.getTag(Constants.POINT_TAG);
        Point dragPoint = (Point) dragImage.getTag(Constants.POINT_TAG);
        Animator animatorOnDragX = ObjectAnimator.ofFloat(onDragImage, "x",
                dragPoint.x * onDragImage.getMeasuredWidth());
        Animator animatorOnDragY = ObjectAnimator.ofFloat(onDragImage, "y",
                dragPoint.y * onDragImage.getMeasuredHeight());
        Animator animatorDragX = ObjectAnimator.ofFloat(dragImage, "x",
                onDragPoint.x * dragImage.getMeasuredWidth());
        Animator animatorDragY = ObjectAnimator.ofFloat(dragImage, "y",
                onDragPoint.y * dragImage.getMeasuredHeight());


        AnimatorSet animatorSet = new AnimatorSet();
        animatorSet.playTogether(animatorDragX, animatorDragY,
                animatorOnDragX, animatorOnDragY);
        animatorSet.start();
    }
